...
public class AnalyticsQueryService {
    ...
    public AnalyticsSummaryDto getSummary(LocalDate from, LocalDate to) {
        ...
        MapSqlParameterSource params = params(normalizedFrom, normalizedTo);

        long leaveRequests = singleLong("""
            SELECT COUNT(*)
            FROM leave_requests lr
            WHERE lr.submitted_at >= :fromAt
        ...
        long pendingLeaveRequests = singleLong("""
            ...
            """, copy(params).addValue("pendingStatuses", LEAVE_PENDING_STATUSES));
        ...
        long pendingAdminRequests = singleLong("""
            ...
            WHERE ar.created_at >= :fromAt
              AND ar.created_at < :toExclusiveAt
              AND ar.status IN (:pendingStatuses)
            """, copy(params).addValue("pendingStatuses", ADMIN_PENDING_STATUSES));
    ...
    @Transactional(readOnly = true)
    public AnalyticsLeaveRequestReportDto getLeaveRequests(LocalDate from, LocalDate to) {
        LocalDate normalizedFrom = normalizeFrom(from, to);
        LocalDate normalizedTo = normalizeTo(from, to);
        MapSqlParameterSource params = params(normalizedFrom, normalizedTo);
    ...
    @Transactional(readOnly = true)
    public AnalyticsLeaveBalanceReportDto getLeaveBalances(LocalDate from, LocalDate to) {
        LocalDate normalizedFrom = normalizeFrom(from, to);
        LocalDate normalizedTo = normalizeTo(from, to);
        MapSqlParameterSource params = params(normalizedFrom, normalizedTo);
    ...
    @Transactional(readOnly = true)
    public AnalyticsApprovalReportDto getApprovals(LocalDate from, LocalDate to) {
        LocalDate normalizedFrom = normalizeFrom(from, to);
        LocalDate normalizedTo = normalizeTo(from, to);
        MapSqlParameterSource params = params(normalizedFrom, normalizedTo);
    ...
    @Transactional(readOnly = true)
    public AnalyticsAdminRequestReportDto getAdminRequests(LocalDate from, LocalDate to) {
        LocalDate normalizedFrom = normalizeFrom(from, to);
        LocalDate normalizedTo = normalizeTo(from, to);
        MapSqlParameterSource params = params(normalizedFrom, normalizedTo);
    ...
    @Transactional(readOnly = true)
    public AnalyticsDashboardDto getDashboard(LocalDate from, LocalDate to) {
        AnalyticsSummaryDto summary = getSummary(from, to);
        MapSqlParameterSource params = params(normalizeFrom(from, to), normalizeTo(from, to));
    ...
    public AnalyticsOverviewDto getOverview(LocalDate from, LocalDate to) {
        ...
        List<AnalyticsLeaveTypeOverviewDto> leaveByType = buildLeaveTypeOverview(params);
        List<AnalyticsOverviewBreakdownDto> bottlenecks = buildApprovalOverview(params);
        List<AnalyticsOverviewBreakdownDto> leaveReasons = buildLeaveReasonOverview(params);
    ...
    @Transactional(readOnly = true)
    public LeaveMetricsSnapshotDto getLeaveMetrics(LocalDate from, LocalDate to) {
        AnalyticsLeaveRequestReportDto report = getLeaveRequests(from, to);
    ...
    @Transactional(readOnly = true)
    public HeadcountMetricsSnapshotDto getHeadcountMetrics(LocalDate from, LocalDate to) {
        LocalDate normalizedFrom = normalizeFrom(from, to);
        LocalDate normalizedTo = normalizeTo(from, to);
        MapSqlParameterSource params = params(normalizedFrom, normalizedTo);
    ...
    @Transactional(readOnly = true)
    public List<LeaveDistributionSnapshotDto> getLeaveDistribution(LocalDate from, LocalDate to) {
        LocalDate normalizedFrom = normalizeFrom(from, to);
        LocalDate normalizedTo = normalizeTo(from, to);
        MapSqlParameterSource params = params(normalizedFrom, normalizedTo);
    ...
    @Transactional(readOnly = true)
    public List<LeaveMetricsTimeseriesPointDto> getLeaveMetricsTimeseries(LocalDate from, LocalDate to) {
        LocalDate normalizedFrom = normalizeFrom(from, to);
        LocalDate normalizedTo = normalizeTo(from, to);
        MapSqlParameterSource params = params(normalizedFrom, normalizedTo);
    ...
    @Transactional(readOnly = true)
    public List<ProjectAbsenceFactDto> getProjectAbsenceFacts() {
    ...
    private List<AnalyticsOverviewSeriesPointDto> buildHeadcountOverviewTrend(LocalDate firstMonth, LocalDate to) {
    ...
    private List<AnalyticsLeaveTypeOverviewDto> buildLeaveTypeOverview(MapSqlParameterSource params) {
    ...
    private List<AnalyticsOverviewBreakdownDto> buildApprovalOverview(MapSqlParameterSource params) {
    ...
    private List<AnalyticsOverviewBreakdownDto> buildLeaveReasonOverview(MapSqlParameterSource params) {
        List<AnalyticsCountDto> rows = jdbcTemplate.query("""
